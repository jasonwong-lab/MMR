...
ci<-mydata[,1:3]
#dev.new()
pdf("mss_forestplot.pdf",5,3)
forestplot(tabletext,rbind(rep(NA, 3),ci),col=clrs,xticks = c(0.8,0.9,1,1.1,1.2),xlog=TRUE,new_page = TRUE,hrzl_lines = list("2" = gpar(lty=1)),vertices = TRUE,xlab="Hazard Ratio",graph.pos = 3,txt_gp = fpTxtGp(label = list(gpar(fontfamily = ""),gpar(fontfamily = "",col = "#660000")),ticks = gpar(cex=1),xlab  = gpar(cex = 1)),title="MSS")
dev.off()