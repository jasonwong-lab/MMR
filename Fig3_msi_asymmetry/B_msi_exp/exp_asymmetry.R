muts<-scan("data/muts_sap",what="s") #n=18
mutl<-scan("data/mutl_sap",what="s") #n=197

p3<-read.table("data/prime3.txt",row.names = 1)
p5<-read.table("data/prime5.txt",row.names = 1)

#===============get muts data==================
muts.p5<-apply(p5[muts,],2,sum)/100
muts.p3<-apply(p3[muts,],2,sum)/100

#===============get mutl data==================
mutl.p5<-apply(p5[mutl,],2,sum)/100
mutl.p3<-apply(p3[mutl,],2,sum)/100
...
dev.new()
cairo_pdf("exp_rep_asym.pdf",4,4)
...
barplot(muts.data,beside=F,col=cl,border=cl,space= rep(rep(c(1,0.2),6),2),axisnames=F,axes=T,ylab="Mutations")
mtext(paste0("MutS","\u03b1","(n=18)"),side=3,adj=0)
barplot(mutl.data,beside=F,col=cl,border=cl,space= rep(rep(c(1,0.2),6),2),axisnames=F,axes=T,ylab="")
mtext(paste0("MutL","\u03b1","(n=197)"),side=3,adj=0)
dev.off()
